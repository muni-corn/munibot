...
| 114 | `feat(harness): add delegation events`                         | `HarnessEvent::DelegationStarted { persona, task }` and `DelegationFinished { persona, ok }`. Delegation must never be invisible — that was the entire objection to automatic routing, and it would be absurd to reintroduce it here. **Landed together with commit 115**: adding these variants alone leaves `munibot_api`'s exhaustive `HarnessEvent` match non-exhaustive, so the two are only ever buildable as one commit — see that row.                                                                                                                                                                                                                                |
| 115 | `feat(api): add delegation to the chat wire types`             | Extend `ChatEvent` and its `HarnessEvent` mapping from milestone 2 phase 11. The mapping is a pure function and is tested directly. **Landed together with commit 114**, in the same commit, for the reason noted there.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
