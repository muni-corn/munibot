...
| 73  | `feat(memory): add conversation summarisation`     | `Summariser` taking a provider and a **`CompactionPersona`, not a full `Persona` as originally planned** — compaction is a single one-shot completion with no tools, budget, or handoff, so the whole type would let a misconfigured compaction role smuggle in a tool allowlist that means nothing here, and would pull `memory` into a dependency on `persona` the crate's own layering forbids (`persona` depends on `memory`, not the reverse). Condenses the oldest messages into prose once history exceeds a token threshold, writing it to `ai_conversations.summary` and deleting the messages it replaced via a new `SessionStore::compact`. Triggered from `Ai::prepare`, right before `assemble_context`, rather than on a timer, so it only costs when it helps. Wired in as an optional `Ai::with_summariser` builder rather than a constructor parameter, so every existing `from_parts`/`new` call site keeps working unchanged. |
