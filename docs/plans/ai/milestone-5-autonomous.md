...
**Phases 20 through 22, commits 152 through 187.**
...
| 171 | `feat(pipeline): add issue analyst role and prompt`        | Write `prompts/pipeline/issue-analyst.md` in full, to the standard of the other eleven: classify as bug, feature, question, or not actionable; attempt reproduction; return `IssueAnalysis` with a recommended action. Runs in a minimal triage sandbox before a full one is provisioned, so a junk issue costs almost nothing.                                      |
| 172 | `feat(pipeline): add researcher and architect prompts`     | Port `codebase-researcher.md` and `software-architect.md`. Preserve the instruction-quality rule verbatim — the demand for `return AppError::NotFound with the message ...` over "add appropriate error handling" is what makes the plans executable by isolated builders.                                                                                           |
| 173 | `feat(pipeline): add plan and code reviewer prompts`       | Port `architecture-reviewer.md`, `code-reviewer.md`, and `final-code-reviewer.md`, fixing the four documented defects. Keep the severity gates exactly as written: critical, major, and minor block; a bare nit does not.                                                                                                                                            |
| 174 | `feat(pipeline): add test engineer and reviewer prompts`   | Port `test-engineer.md` and `test-reviewer.md`. Tests are written **before** the implementation and the builder may not modify them; preserve that framing and the mutation-testing mindset in the reviewer, which is the sharpest quality gate in the whole pipeline.                                                                                               |
| 175 | `feat(pipeline): add builder and project manager prompts`  | Port `builder.md` and `project-manager.md`. Preserve the precedence rule that tests outrank instructions on conflict, and the project manager's `context` field as the only channel carrying cross-subtask memory to an isolated builder.                                                                                                                            |
| 176 | `feat(pipeline): add commit crafter and pr author prompts` | Port `commit-crafter.md` and `pr-author.md`. Adapt both to munibot's own conventions in `AGENTS.md` and `.versionrc`: stage files individually, never amend, re-stage after a formatter hook rewrites a file, no co-author footers, and a scope matching the repository's scope regex.                                                                               |
| 177 | `feat(pipeline): add agent dispatcher over the harness`    | `AgentDispatcher` trait with `invoke_agent(role, context) -> AgentOutput`, and a `HarnessDispatcher` mapping each `AgentRole` to its persona, prompt template, and handoff schema, then delegating to `Harness::run_turn`. A mock dispatcher makes the entire executor testable with no model calls.                                                                 |
| 178 | `feat(pipeline): add branch naming with idempotent reuse`  | `munibot/{issue_number}-{slug}` with the slug lowercased, dash-separated, alphanumeric, and capped at 60 characters. Reuse an existing branch for the same issue; on collision with unrelated work, append an attempt suffix. Pure function, thoroughly tested, because a wrong branch name here means a wrong pull request.                                         |
| 179 | `feat(pipeline): add executor loop with persistence`       | Dispatch the agent for the current state, append the resulting event, advance, persist, repeat until `Complete` or `Failed`. Provision the sandbox on entering `Researching` and tear it down on exit. Every iteration is durable, so a crash resumes rather than restarts.                                                                                          |
...
| #   | Commit                                                   | Description                                                                                                                                                                                                                                                                                                       |
| --- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 180 | `feat(pipeline): add interaction adapter trait`          | `InteractionAdapter` with `request_input(InteractionRequest) -> InteractionResponse` and `notify(pipeline_id, message)`. A `RequestPlanHelp` or `RequestBuildHelp` handoff moves the pipeline to `AwaitingUserInput` and persists, so waiting costs nothing and survives a restart.                               |
| 181 | `feat(pipeline): add github comment interaction adapter` | Post the question as an issue comment and resume when a maintainer replies, matching on the comment thread. The fallback adapter when no Discord channel is configured for a repository.                                                                                                                          |
| 182 | `feat(discord): add pipeline interaction adapter`        | **The reason this lives in munibot.** Open a thread in a configured Discord channel, post the question with context, and resume the pipeline on your reply. Verify the responder has permission over the repository before accepting an answer, since a thread is visible to a whole server.                      |
| 183 | `feat(pipeline): add fix subtask synthesis`              | On `RequestCodeChanges` from the final reviewer, enter `AwaitingFixSubtask`, re-invoke the project manager with the full feedback, receive a `FixSubtask` carrying `review_feedback` and `parent_subtask_id`, and re-enter the test-and-build cycle. Without this the pipeline dead-ends on its own final review. |
| 184 | `feat(pipeline): add concurrency limits and queue`       | `ConcurrencyConfig` with a global and a per-repository maximum, a FIFO queue for overflow, and a `PipelineRegistry` backed by database rows. `abort_pipeline` propagates cancellation into the harness and then stops the container. One runaway repository must not starve every other one.                      |
| 185 | `feat(pipeline): add pipeline resume after restart`      | On startup, load every non-terminal pipeline, replay its events, re-provision its sandbox, and continue. Integration test: kill the process mid-build and assert the pipeline completes on restart. This is the payoff for event sourcing.                                                                        |
| 186 | `feat(gui): add pipeline monitor page`                   | Live pipeline list with state, current subtask, elapsed time, and accumulated cost, streamed over server-sent events. Per-pipeline detail showing the event log, every agent invocation, and every tool call. An unobservable autonomous system is an unusable one.                                               |
| 187 | `feat(discord): add pipeline status commands`            | `/pipeline list`, `/pipeline status <id>`, and `/pipeline abort <id>`, restricted to configured maintainers. Abort matters most: being able to stop a misbehaving run from your phone is worth more than any dashboard.                                                                                           |
