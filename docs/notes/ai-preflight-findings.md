...
## 10. Two more rig API gaps, found while implementing rather than pre-flighting

The pre-flight checks above ran before any code was written. Building the rig adapter and error
classifier (`munibot_ai/src/provider/rig/adapter.rs`, `errors.rs`) surfaced two further gaps of the
same shape as findings 1 and 2 — real limitations of rig's generic, provider-agnostic types, each
with a documented workaround rather than a silent guess.

**No normalized stop/finish reason.** Neither `CompletionResponse<T>` nor
`StreamingCompletionResponse<T>` exposes a finish reason at the generic level — only the
provider-specific `raw_response: T` might carry one, which would defeat the point of a
provider-agnostic type. The adapter infers `StopReason` from content alone: `ToolUse` if the response
contains any tool call, `EndTurn` otherwise. This does not affect harness correctness, since the loop
only ever branches on `ToolUse` versus everything else, but it does mean `MaxTokens`, `StopSequence`,
and `Refusal` are not currently distinguishable through this adapter.

**No response headers in errors.** `CompletionError` exposes a public `provider_response_status()`
helper (generated by an internal macro) returning `Option<http::StatusCode>`, which is enough to
classify a failure as a rate limit, a transient server error, or a permanent rejection. But nothing in
rig's error model preserves response headers, so a `Retry-After` value can never be extracted — every
retry falls back to its own computed backoff, never the provider's own guidance. This is also why
