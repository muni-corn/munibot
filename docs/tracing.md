...
| `munibot_ai::service`                      | `turn{persona, user_id, guild_id, platform}`                         | `Ai::turn`, one full turn                                                                                                                                                                                                  |
| `munibot_ai::service`                      | `turn_streamed{persona, user_id, guild_id, platform}`                | `Ai::turn_streamed` - covers setup only (persona resolution, rate limiting, context assembly); the stream itself keeps running after this span ends, since `tracing`'s `Instrument` has no `Stream` support, only `Future` |
| `munibot_ai::service`                      | `delegate{persona, delegation_depth}`                                | `Ai`'s `Delegator` impl, one nested turn                                                                                                                                                                                   |
| `munibot_ai::service`                      | `generate_conversation_title{conversation_id}`                       | Spawned background task                                                                                                                                                                                                    |
| `munibot_ai::service`                      | `post_hoc_moderation_check{conversation_id, user_id}`                | Spawned background task, streamed turns only                                                                                                                                                                               |
| `munibot_ai::harness`                      | `run_turn_recording_usage{model, conversation_id, delegation_depth}` | `Harness`'s shared turn loop, both `run_turn` and `run_turn_recording_usage`                                                                                                                                               |
| `munibot_ai::pipeline::executor`           | `run{pipeline_id}`                                                   | One executor pass over a pipeline's own state                                                                                                                                                                              |
